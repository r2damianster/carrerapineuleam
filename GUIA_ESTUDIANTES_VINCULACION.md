# Guía del Portal PINE para Pasantes de Vinculación

> Documento base para capacitar a estudiantes-instructores (pasantes) de Vinculación. Cada sección sigue el mismo formato: **qué es** → **paso a paso con clics exactos** → **qué pasa después**. Pensado para convertirse directamente en diapositivas (1 sección ≈ 1-2 diapositivas).

---

## 0. Antes de empezar: conceptos clave

- **Espacio** = el club/aula donde trabajas (ej. "Club de Inglés A"). Lo crea tu profesor y te asigna como instructor. Todo lo que registras (asistencia, beneficiarios, tests, encuestas, difusión) se hace **dentro de un espacio**.
- **Beneficiario** = la persona externa (niño, joven, adulto de la comunidad) que recibe el club/clase. El beneficiario **nunca tiene cuenta ni contraseña** — solo tú y tu profesor lo registran y gestionan.
- **Tú (pasante)** sí tienes cuenta en el Portal, creada por tu profesor con tu nombre y correo. La activas la primera vez que entras.

---

## 1. Primer ingreso y activación de tu cuenta

**Qué es:** tu profesor ya te registró en el sistema con tu nombre y correo. No existe pantalla de "registro" para ti — activas tu cuenta la primera vez que intentas entrar.

**Pasos:**
1. Ir a **`/portal/login`** (Portal PINE → botón "Ingresar al Portal").
2. En **Correo Electrónico**, escribir el correo que tu profesor registró para ti.
3. En **Contraseña**, escribir la clave que **tú quieras usar de ahora en adelante** — la primera vez que la escribas, el sistema la guarda como tu contraseña definitiva.
4. Clic en **"Ingresar al Portal"**.

**Qué pasa después:** entras directo a tu **Dashboard** (`/portal/dashboard`). Las siguientes veces, usas esa misma contraseña para entrar — si la olvidas, pide a tu profesor que te reactive.

⚠️ Si te equivocas al escribir la contraseña la primera vez, esa quedó guardada — no hay forma de "reintentar", debes pedir a tu profesor que te reactive la cuenta.

---

## 2. Tu Dashboard (pantalla de inicio)

Al entrar verás tarjetas según lo que tengas asignado. Como pasante de Vinculación verás normalmente:

- **Registros de Vinculación** — asistencia, beneficiarios, test MCER, encuesta, difusión (el corazón de tu trabajo diario, ver secciones 3-7).
- **📊 Mi Avance** — tus horas acreditables (incluye horas de podcast), espacios asignados, beneficiarios y evaluaciones que ya registraste. Botón **"» Ver mi avance completo"** → `/portal/mi-avance`.
- **Investigación (Vinculación)** *(solo si tu profesor te habilitó este módulo)* — botón **"» Reportar Actividades"** para registrar horas/actividades de investigación aparte de tus horas de vinculación.

---

## 3. Registrar Asistencia

**Ruta:** Dashboard → tarjeta "Registros de Vinculación" → **"» Registrar Asistencia"** (`/vinculacion/asistencia`)

**Pasos:**
1. **Espacio** → selecciona tu club/aula del desplegable.
2. **Fecha** → elige la fecha de la sesión.
3. **Beneficiarios presentes** → marca con ✅ (checkbox) a cada beneficiario que asistió ese día. Si no ves a nadie en la lista, primero debes inscribirlo (ver sección 4).
4. **Observaciones** (opcional) → texto libre.
5. Clic en **"Guardar Asistencia"**.

---

## 4. Registrar/Asignar Beneficiarios

**Ruta:** Dashboard → "Registros de Vinculación" → **"» Registrar Beneficiarios"** (`/vinculacion/beneficiarios`)

**Primero:** selecciona tu **Espacio** en el desplegable de arriba.

Hay **2 pestañas**:

### 4.1 Pestaña "Asignar existente"
Úsala cuando el beneficiario **ya está registrado** en el sistema (de otro espacio o ciclo) y solo necesita entrar al tuyo.
1. Marca con ✅ los beneficiarios que quieres agregar a tu espacio (lista con checkboxes).
2. Clic en **"Asignar Seleccionados"**.
3. Abajo puedes ver "Ya inscritos en este espacio" con el conteo actual.

### 4.2 Pestaña "Registrar nuevo"
Úsala cuando el beneficiario **nunca ha estado en el sistema**.
1. **Nombres** / **Apellidos** (obligatorio).
2. **Contacto (teléfono)**.
3. **Email** (opcional).
4. **Edad**.
5. Checkbox **"Tiene discapacidad"** → si lo marcas, aparece un campo **"¿Cuál?"**.
6. **Situación ocupacional** → elige una opción:
   - *Solo estudia*
   - *Estudia y trabaja*
   - *Solo trabaja*
   - *Desempleado y no estudia*
   - Si eligió una opción "trabaja" → aparece **"Rol que ejerce"**.
   - Si eligió una opción "estudia" → aparece **"Nivel educativo"** (Universidad / Colegio / Escuela); si es Universidad, además **"Carrera"** y **"Curso/semestre"**.
7. Clic en **"Registrar y Asignar a este Espacio"**.

> El beneficiario queda automáticamente inscrito en el espacio que elegiste arriba — no hace falta un paso extra.

---

## 5. Aplicar el Test de Nivelación MCER

**Ruta:** Dashboard → "Registros de Vinculación" → **"» Test MCER"** (`/vinculacion/test-mcer`)

**Pasos:**
1. **Espacio** → selecciona el club/aula (si solo tienes uno asignado, se autoselecciona).
2. **Beneficiario Evaluado** → elige de la lista.
3. **Momento del Test** → elige:
   - **Pre-Test (Inicial)** → al empezar el ciclo/club.
   - **Post-Test (Final)** → al terminar el ciclo/club.
4. **Foto / Evidencia Física** (opcional) → si aplicaste el test en papel, sube la foto.
5. Responde **todas** las preguntas del test (opción múltiple + una pregunta de audio que se graba con el micrófono — botón de grabar, máximo el tiempo indicado).
6. Clic en **"Enviar y Evaluar"**.

**Qué pasa después:** el sistema calcula el puntaje y el nivel MCER automáticamente y te muestra el resultado en pantalla (puntaje /100, desglose de Gramática/Lectura/Oral, nivel asignado).

### 5.1 Alternativa: enlace/QR sin login (el beneficiario responde desde su celular)
- Botón **"🔗 QR Pre-Test (sin login)"** (arriba de la página, se habilita cuando elegiste un espacio) → genera un link/QR temporal para que el beneficiario tome el pre-test él mismo, sin necesitar cuenta.
- Botón **"🔗 QR Post-Test (sin login)"** (se habilita cuando ya elegiste un beneficiario en el paso 2) → genera un link/QR de un solo uso para el post-test de esa persona.
- En ambos casos: clic en el botón → aparece un modal con el QR y el link → puedes copiarlo o compartirlo por WhatsApp.

---

## 6. Encuesta de Satisfacción

**Ruta:** Dashboard → "Registros de Vinculación" → **"» Encuesta"** (`/vinculacion/encuesta`)

**Pasos:**
1. **Espacio** → selecciona tu club/aula.
2. **Beneficiario** → a quién le tomas la encuesta.
3. **Ciclo / Semestre a evaluar** → selecciona el ciclo académico.
4. Califica con **estrellas (1-5)** cada pregunta:
   - ¿Qué tan satisfecho está el beneficiario con el programa?
   - ¿Sintió que aprendió?
   - ¿Sintió que mejoró su nivel de inglés?
   - ¿Cómo calificaría los recursos/materiales usados?
5. **Calificación por instructor** → aparece automáticamente una fila de estrellas por cada estudiante-instructor del espacio (incluyéndote a ti si eres uno de ellos) — el beneficiario califica a cada instructor.
6. **Comentarios adicionales** (opcional) → texto libre.
7. Clic en **"Enviar Encuesta"**.

### 6.1 Alternativa QR (igual que en MCER)
Botones **"🔗 QR Pre-Encuesta"** y **"🔗 QR Post-Encuesta"** arriba de la página — mismo mecanismo: genera link/QR para que el beneficiario responda desde su propio celular sin cuenta.

---

## 7. Registrar Difusión (Evento o Podcast)

**Ruta:** Dashboard → "Registros de Vinculación" → **"» Difusión / Evento"** (`/vinculacion/difusion`)

**Pasos:**
1. **Título del Evento / Podcast**.
2. **Tipo de Difusión** → elige:
   - Podcast
   - Evento Físico
   - Encuentro Comunitario
   - Evento de Formación
   - Visita Técnica
3. **Fecha**.
4. **Audiencia Alcanzada (N° Personas)** → número de personas que participaron o vieron/escucharon.
5. **Profesor(es) Responsable(s)** → marca con ✅ al menos un profesor de la lista (obligatorio).
6. **Si el tipo es "Podcast"** aparecen 2 secciones extra:
   - **Video del podcast**: elige una **categoría** del desplegable, selecciona el **archivo de video**, clic en **"Subir video"** (se sube directo a YouTube, con barra de progreso). Es obligatorio subirlo antes de registrar la actividad.
   - **Participantes del podcast**: marca con ✅ los pasantes que aparecieron en el episodio; agrega **invitados internos** (de la universidad) y/o **invitados externos** escribiendo el nombre y clic en "Agregar".
7. **Foto / Evidencia del Evento o Podcast** → obligatorio subir una foto (captura de métricas del podcast, o foto del evento físico).
8. Clic en **"Registrar Actividad de Difusión"**.

**Qué pasa después:** la actividad queda **pendiente de aprobación** por el administrador del sitio antes de aparecer en la web pública — es normal que no la veas de inmediato en `carrerapineuleam.vercel.app`.

### 7.1 Horas acreditables por podcast (solo Vinculación)
Si marcaste tu propio nombre en "Participantes del podcast", el sistema calcula automáticamente tus horas acreditables según esta tabla — **no hay que pedirlas aparte**:

| Categoría (se detecta sola según los invitados que agregaste) | Horas |
|---|---|
| Solitario / Entre estudiantes (sin invitados) | 3 h |
| Con invitado interno | 5 h |
| Con invitado externo | 8 h |
| Panelista adicional interno / externo | +1 h / +2 h cada uno |
| Bono audiencia 1-10 / 11-20 / más de 20 personas | +1 h / +2 h / +3 h |

⚠️ Las horas **no se acreditan de inmediato** — quedan pendientes hasta que el profesor/admin aprueba el video en el panel de administración. Mientras tanto verás el aviso "episodio(s) esperando aprobación" en tu tarjeta "Mi Avance".

---

## 8. Ver mi avance (horas, espacios, evaluaciones)

**Ruta:** Dashboard → tarjeta **"📊 Mi Avance"** → **"» Ver mi avance completo"** (`/portal/mi-avance`)

Ahí puedes revisar en un solo lugar: tus horas acreditadas (incluye podcast), en qué espacios estás asignado, qué beneficiarios has registrado/atendido, y las evaluaciones (MCER/encuestas) que ya cargaste.

---

## 9. Reportar Actividades de Investigación (solo si te habilitaron este módulo)

**Ruta:** Dashboard → tarjeta "Investigación (Vinculación)" → **"» Reportar Actividades"** (`/vinculacion/investigacion-actividades`)

Úsalo únicamente si tu profesor te asignó también funciones de investigación además de vinculación — es un reporte de horas/actividades aparte, no reemplaza nada de las secciones 3-7.

---

## Resumen visual (para la diapositiva de cierre)

```
Portal PINE (login con correo institucional)
        │
        ▼
   Dashboard
        │
        ├─ Registros de Vinculación
        │     ├─ Registrar Asistencia
        │     ├─ Registrar Beneficiarios (Asignar existente / Registrar nuevo)
        │     ├─ Test MCER (Pre/Post + QR sin login)
        │     ├─ Encuesta (estrellas + calificación por instructor + QR sin login)
        │     └─ Difusión / Evento (podcast → video + participantes → horas acreditables)
        │
        ├─ 📊 Mi Avance (horas, espacios, beneficiarios, evaluaciones)
        │
        └─ Investigación (Vinculación) — solo si aplica
```

---

**Documento generado:** 2026-09-16, a partir del código real de `carrerapineuleam` (rutas, campos y textos exactos de la interfaz vigente en producción).
