...
- **Migración de `lib/data.ts` → Neon: completa.** Las 6 entidades del sitio público (members, video_categories+videos, news+activities+difusión, publications) ya persisten en Neon. `lib/db.ts` queda con las funciones de esas entidades sin ningún caller (confirmado con `grep "from '@/lib/db'"` — solo quedan `getSiteSettings`/`updateSiteSetting` en `/admin/settings` y `getNewsletters` en `/boletines`, ambos vigentes: `SiteSettings` no se migró en este pase, quedó fuera de alcance desde el plan original; `getNewsletters` se reescribió para leer de la Neon nueva, no del array estático). **Pendiente de limpieza** (no bloqueante, cosmético): borrar de `lib/db.ts` las funciones CRUD de member/publication/videoCategory/video/news/activity que ya nadie llama, y de `lib/data.ts` los arrays `members`/`publications`/`videoCategories`/`videos`/`news`/`activities` — dejando solo `siteSettings`, `profesoresAutorizados`, `profesorModulos`, `liderProyectoPropio`, `vinculacionEnglishClubPhotos`.
