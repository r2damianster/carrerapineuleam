...
- **Orden de equipos:** `proyecto_miembros.orden` se fijó por rol (líder, colíder, supervisor, vinculación) y luego participantes por apellido.
- **Líder de proyecto propio:** las tarjetas "Gestionar {proyecto}" del dashboard salen de `proyectos.lider_id` (tipo `plantilla_simple`); se eliminó `liderProyectoPropio` de `lib/data.ts`.
- **Beneficiarios normalizados** a "Nombre Apellido" (respaldo: `respaldo_beneficiarios_nombres_20260925`, borrable).
- **Duplicados de beneficiarios fusionados (2026-09-25, decisión del usuario):** se conservaron 101, 103, 66, 98 y 112 y se retiraron 63, 64, 102, 96 y 111 (mismo espacio, mismas asistencias). Se trasladaron perfil, inscripción y asistencia al registro conservado (en 112 se tomó la carrera bien escrita del 111); de cada par queda **un solo pre-test** (el del registro conservado). También se eliminaron los registros de prueba 27 ("Prueba QA Beneficiario Uno") y 28. Todo lo movido o borrado (usuarios, perfiles, inscripciones, evaluaciones, asistencias) está respaldado en `respaldo_duplicados_beneficiarios_20260925` (jsonb por tabla; borrable). Quedan sin tocar: el espacio de prueba "PRUEBA QA - Club de Inglés" y su asistencia, correo inválido del id 114, nombres sin partir de los ids 62 y 77, y `genero` en NULL para todos los beneficiarios (usar `/vinculacion/beneficiarios/genero`).
- **Género obligatorio en el registro de beneficiarios (2026-09-25):** `usuarios.genero` (`femenino|masculino|otro|prefiero_no_decir`, lista en `lib/generos.ts`) es requerido al crear un beneficiario nuevo, tanto en `/vinculacion/registrar-evaluar` (`POST /api/beneficiarios/registrar-y-evaluar`) como en el enlace/QR público (`POST /api/enlaces/[token]/pretest`, que además lo completa si quien elige "Ya estoy registrado" aún no lo tenía; nunca sobrescribe). La plantilla Word del pre-test trae la casilla de género. La herramienta temporal `/vinculacion/beneficiarios/genero` y su API se **eliminaron**. El género de las personas ya existentes (beneficiarios, pasantes, docentes, directorio; `members.genero` sincronizado) se asignó por nombre; **queda vacío solo el id 67 (Americk Ramos Dúque)** a la espera de confirmación.
- **Nombres normalizados también al registrar:** `lib/nombres.ts:normalizarNombrePropio` (mayúscula inicial por palabra, partículas `de/del/la…` en minúscula, sin espacios sobrantes) se aplica a nombres y apellidos de beneficiarios nuevos (panel y público). Antes entraban tal cual se escribían (ej. id 130 con doble espacio).
- **Portada (`/`):** el encabezado con el nombre de la carrera (`HubProjectsSection`, ya sin tarjeta) va antes de "Perfiles de la Carrera" y luego "Últimas noticias". Boletines dejó de ser tarjeta: se enlaza con "Ver boletines →" a la derecha del título de noticias (`t.news.viewBulletins`) y en los enlaces rápidos del pie de la portada (`footerContexts.landing`, clave `newsletters`). Subtítulos: `hub.sectionSubtitle` y `news.sectionSubtitle` ("novedades de la carrera"). Se quitaron las claves `hub.groupName/careerLabel/pineCard/boletinesCard` (sin uso).
- **Líder de proyecto: una sola fuente (2026-09-25, decisión del usuario).** El líder sale siempre del rol `lider` en `proyecto_miembros` (`lib/liderProyecto.ts`). `proyectos.lider_id/lider_nombre/lider_email/lider_orcid` son columnas **heredadas**: ya no se escriben; `GET /api/proyectos`, `GET /api/proyectos/[slug]` y la ficha de Vinculación las rellenan desde el equipo (correo y ORCID desde la tarjeta pública `members`). `/admin/proyectos` y la ficha (`/vinculacion/proyecto`) muestran el líder como solo lectura; se cambia en `/admin/members` (rol Líder). El dashboard arma "Gestionar {proyecto}" desde el equipo.
- **Permisos por pertenencia a proyectos (2026-09-25):** `lib/permisosPertenencia.ts`. Reglas: supervisor o líder del proyecto `vinculacion` → módulo `vinculacion`; líder de `vinculacion` → `vinculacion_gestion`; líder de un proyecto del área `investigacion` → `investigacion`. Solo docentes (`profesor`/`admin`); admin, contenido_sitio, superadmin y subir_video siguen siendo manuales. `usuarios.modulos_acceso` es el **resultado efectivo**: `(derivados ∪ modulos_manuales) − modulos_excluidos`; se recalcula al guardar el equipo de una persona (`sincronizarProyectosDePersona`). `/admin/roles` puede sobrescribir: marcar un módulo lo guarda en `modulos_manuales`, desmarcar uno derivado lo guarda en `modulos_excluidos` (la casilla derivada muestra "auto"). Migración aplicada en Neon (columnas `modulos_manuales`/`modulos_excluidos`, inicializadas para que el permiso efectivo de todos quedara idéntico; respaldo en `respaldo_modulos_acceso_20260925`, borrable). ⚠️ **Todo código que escriba `modulos_acceso` de un docente debe actualizar también `modulos_manuales`/`modulos_excluidos`** (como hacen `/api/admin/roles` y el registro), o el siguiente recálculo lo revertirá.
- **Enlaces públicos con token mal formado** (`/api/enlaces/**`, `/api/enlaces-difusion/**`) responden 404 (`lib/tokens.ts`); antes daban 500 por el cast a uuid. **Orden por proyecto editable** en `/admin/members` (campo "Orden" junto al rol de cada proyecto, `proyecto_miembros.orden`). Corregido el correo truncado del beneficiario 114.
- **`CareerProfileSection` (perfil de egreso) migrada a i18n:** todo su texto vive ahora en `lib/i18n.tsx` → `t.careerProfile` (ES y EN, 46 pares); el componente conserva solo estructura y estilos (`estilosCompetencias`). Se corrigió de paso la etiqueta fija "COMPETENCIA" (salía en español también en inglés): ahora `t.careerProfile.egreso.competencyLabel`. Para editar el texto de un dominio, cambiar `careerProfile.competencias[n]` en ambos idiomas.
- **Selector de docentes de la ficha del proyecto** (`/vinculacion/proyecto`): lista a todo `usuarios.rol IN ('profesor','admin')` sin exigir `activado` (muchos docentes aún no activan su cuenta) y ordenada por apellido. Las autoridades externas (rol NULL) y la secretaria no aparecen como responsables/líder.
- **Incidente de auto-commit (2026-09-25):** la carpeta de trabajo compartida quedó con `app/api/admin/stats/route.ts` con marcas de conflicto (`<<<<<<<`) sin resolver y el hook de auto-commit las subió a `main` dos veces (deploys en ERROR). **Antes de cerrar sesión, `grep -rn "<<<<<<<" app lib components` debe dar 0.** Los archivos de 0 bytes también rompen el build (`is not a module`): un archivo nuevo no se commitea vacío.
- **Revisión del trabajo de Antigravity (informes de Vinculación, 2026-09-25):** en `main` entraron 12 archivos de 0 bytes (`app/vinculacion/informes/**`, `app/portal/documentos/**`, `app/api/espacios/actividades/route.ts`, `lib/documentos-docentes.ts`, `lib/informesVinculacion.ts`, dos scripts) que rompían el build (`File ... is not a module`, deploys en ERROR). Se quitaron los vacíos y se restauró `lib/informesVinculacion.ts` (17 KB, versión de `e81ac7a`). Correcciones a su código: `GET /vinculacion/proyecto/api` fallaba con `column "activo" does not exist` (`ciclos_academicos` no tiene esa columna); la regla de notificación `informe-supervisor-pendiente` comparaba `informes_vinculacion.mes` (tipo `date`) con `'YYYY-MM'` y apuntaba a una página inexistente (queda apagada con `INFORMES_VINCULACION_HABILITADOS = false`); `/vinculacion/proyecto` y `/vinculacion/beneficiarios/genero` no estaban en `protectedRoutes`. Pendiente suyo: páginas y generadores de informes (`docxSupervisor`, `docxLider`, `graficos`), `portal/documentos`, y mover el texto de `CareerProfileSection` a `lib/i18n.tsx` (usa `lang === 'es'` en línea).
- **Pendiente de esta línea:**  (lib/data.ts) y las tarjetas "Gestionar {proyecto}" del dashboard a `proyecto_miembros`/`proyectos.lider_id`; derivar permisos de la pertenencia; orden por proyecto editable en el admin (hoy hereda el orden global al crear).
...
10b. **Flujo obligatorio de ramas (decidido 2026-09-25):** cada agente trabaja en **su propia rama y `git worktree`**, nunca editando el árbol principal `carreraPINE` a la vez que el otro. Se integra a `main` solo con `npx tsc --noEmit` y `npm run build` en verde y sin marcas de conflicto (`grep -rn "<<<<<<<" app lib components` = 0) ni archivos de 0 bytes. Si el hook de auto-commit está activo, el árbol principal debe quedar limpio: un archivo a medias se sube a `main` y rompe el deploy (pasó dos veces el 2026-09-25). Un `git worktree` necesita enlazar `node_modules` (junction) y copiar `.env.local`; al terminar, quitar el junction con `cmd /c rmdir` **antes** de `git worktree remove` (nunca borrar recursivo a través del enlace).
