...
✅ **Primeros 8 registros reales cargados** (artículos regionales de Arturo, Jhonny y Cristina Basantes) vía `scripts/seed-articulos-regionales.js` — Prisma directo contra la Neon de producción (mismo `DATABASE_URL` que usa el deploy), no por el wizard HTTP (no había forma de autenticar una sesión de navegador desde este entorno). Sirvió para confirmar que el schema/enum acepta los datos reales antes de dejar el módulo en manos del equipo.
