...
- ⏳ **Pendiente**: `publications` (requiere auditoría de contenido con el usuario primero — hay publicaciones 2025 dudosas y 2026 faltantes en la lista estática actual). Es la última entidad — al terminarla, limpiar `lib/data.ts`/`lib/db.ts` de las funciones ya sin uso (member/video/news/activity), dejando solo `profesoresAutorizados`/`profesorModulos`/`liderProyectoPropio`/`vinculacionEnglishClubPhotos`/`siteSettings`.
- Scripts de migración: `scripts/migrate-members.js`, `scripts/migrate-videos.js`, `scripts/migrate-actividades-difusion-extend.js` + `scripts/seed-news-activities-legacy.js` (patrón a seguir para `publications`).
