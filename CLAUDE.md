...
- ℹ️ **Pendiente, decidido en concepto pero no implementado:** el rol `estudiante` (instructor) debería tener lista fija igual que `profesor` (`estudiantesAutorizados`, mismo patrón que `profesoresAutorizados`) — falta que el usuario dé los emails reales, hoy `estudiante` sigue con autoregistro abierto. Y `beneficiario` **no debería tener cuenta/password en absoluto** — el usuario aclaró que "no tienen acceso, solo quedan registrados": es el instructor/profesor quien los da de alta como dato (nombre/contacto/situación laboral), no ellos mismos vía `/registro`. Falta diseñar dónde vive ese formulario (candidato: tab "Beneficiarios" dentro de `/vinculacion/espacios/[id]`) — el usuario pidió dejarlo "como está" por ahora. **No tocar el rol `beneficiario` en `/registro` sin retomar esta conversación primero.**
