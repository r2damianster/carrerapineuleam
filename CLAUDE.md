...
- **"Espacio" = Gestión, solo profesor** (hoy: Arturo, Cynthia). `/vinculacion/espacios` crea/lista espacios (ej. "Club de Inglés A"); `/vinculacion/espacios/[id]` asigna estudiantes como instructores de ese espacio (única función que queda ahí). `/vinculacion/pasantes` es la vista agregada de todos los estudiantes-instructores y en qué espacio(s) está cada uno.
...
- ℹ️ **Pendiente, decidido en concepto pero no implementado:** el rol `estudiante` (instructor) debería tener lista fija igual que `profesor` (`estudiantesAutorizados`, mismo patrón que `profesoresAutorizados`) — falta que el usuario dé los emails reales, hoy `estudiante` sigue con autoregistro abierto.
