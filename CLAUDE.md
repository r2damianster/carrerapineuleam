...
- ℹ️ **Nota (Sesión 24, confirmada con el usuario):** la persona real `veronica.chavez@uleam.edu.ec` (Verónica Chávez Zambrano) sí lidera un proyecto propio — nombre oficial **"Mentoring"**, distinto de la "Mentoría Lingüística" fabricada arriba. Ya estaba en `lib/data.ts:liderProyectoPropio` (junto con `'Desarrollo de Habilidades Lingüísticas'` de German) pero sin página pública ni mención en este changelog. Son 4 proyectos del grupo en total: Innovaciones Pedagógicas e Internacionalización (Arturo/Jhonny), Desarrollo de Habilidades Lingüísticas (German), Dinámicas Lingüísticas en Contextos Locales / Vinculación (Cynthia), Mentoring (Verónica) — usados en el dropdown "Proyecto" del wizard de Contribuciones Académicas.
