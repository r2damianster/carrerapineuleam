...
- **"Supervisar actividades y horas" (`/vinculacion/supervisar`)**: pestañas Asistencia | Podcast | Investigación, con filtros de **período académico** (`ciclos_academicos`, por defecto el vigente), **estado** y, solo para líder/superadmin, **"Pasantes de"** (todos los supervisores / solo los míos / un supervisor). Un supervisor regular siempre ve solo sus pasantes. `GET /api/vinculacion/supervisar-horas?tipo=podcast|investigacion` y `PATCH .../supervisar-horas/[tipo]/[id]` (aprobar/rechazar con motivo). Alcance en `lib/alcanceSupervision.ts`. Regla de pertenencia: el supervisor de un pasante es el `profesor_id` de los espacios donde ese pasante es instructor.
...
- **Tarjetas del dashboard:** Registros de Vinculación (pasante, Cintya, superadmin; "Registrar podcast" solo con `subir_video`), Supervisión de Vinculación (supervisores y líder), Gestión de Vinculación (líder), Gestionar Investigación (`investigacion`), Roles y usuarios (`admin`), Administrador de sitio, Superadmin. El aviso "sin módulos" ya no sale a docentes.
